...
$topNotifications = $user ? array_slice(Notification::all((int) $user['id']), 0, 5) : [];
...
<body class="<?= $user ? 'app-page' : 'auth-page' ?>">
    ...
    <?php if (!$user): ?>
        <button class="theme-toggle" id="themeToggle" aria-label="Toggle theme"><?= icon_svg('theme') ?></button>
    <?php endif; ?>
    ...
    <?php if ($user): ?>
        ...
        <div class="top-actions">
            <a class="icon-button" href="messenger.php" aria-label="Open messenger" data-count="<?= $messageCount ?>">
                <?= icon_svg('mail') ?>
                <?php if ($messageCount > 0): ?><span class="badge"><?= $messageCount ?></span><?php endif; ?>
            </a>
            <a class="icon-button" href="notifications.php" aria-label="Open notifications" data-count="<?= $notificationCount ?>">
                <?= icon_svg('bell') ?>
                <?php if ($notificationCount > 0): ?><span class="badge"><?= $notificationCount ?></span><?php endif; ?>
            </a>
            <button class="theme-toggle" id="themeToggle" aria-label="Toggle theme"><?= icon_svg('theme') ?></button>
        </div>
