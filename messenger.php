...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['start_direct'])) {
    try {
        $conversationId = Messenger::findOrCreateDirect((int) $user['id'], $_POST['recipient_lookup'] ?? '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_members'])) {
    try {
        Messenger::addMembers((int) $_POST['conversation_id'], (int) $user['id'], $_POST['members'] ?? []);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message'])) {
    try {
        Messenger::send((int) $_POST['conversation_id'], (int) $user['id'], $_POST['message'] ?? '');
        redirect('messenger.php?conversation_id=' . (int) $_POST['conversation_id']);
    } catch (Throwable $exception) {
        $error = $exception->getMessage();
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_message'])) {
    try {
        Messenger::deleteMessage((int) $_POST['message_id'], (int) $user['id']);
        redirect('messenger.php?conversation_id=' . (int) $_POST['conversation_id']);
    } catch (Throwable $exception) {
        $error = $exception->getMessage();
    }
...
$activeId = isset($_GET['conversation_id']) ? (int) $_GET['conversation_id'] : (int) ($conversations[0]['id'] ?? 0);
$activeConversation = $activeId ? Messenger::findConversation($activeId, (int) $user['id']) : null;
$messages = $activeConversation ? Messenger::messages($activeId, (int) $user['id']) : [];
$availableUsers = Messenger::availableUsers((int) $user['id']);
$members = $activeConversation ? Messenger::members($activeId) : [];
...
<section class="messenger-layout messenger-pro">
    <aside class="panel messenger-sidebar reveal fade-up">
        ...
        <div class="conversation-list">
            ...
            <?php endforeach; ?>
            <?php if (!$conversations): ?><p class="muted">No conversations yet.</p><?php endif; ?>
        </div>
            ...
            <label>Members
                <select name="members[]" multiple size="5" required>
                    <?php foreach ($availableUsers as $person): ?>
                        <option value="<?= (int) $person['id'] ?>"><?= e($person['fullname']) ?> · <?= e($person['phone']) ?></option>
                    <?php endforeach; ?>
                </select>
            </label>
        ...
        <?php if ($activeConversation): ?>
            <header class="chat-header">
                <div>
                    ...
                    <p class="muted">
                        <?php foreach ($members as $index => $member): ?><?= $index ? ', ' : '' ?><?= e($member['fullname']) ?><?php endforeach; ?>
                    </p>
                </div>
            </header>
            ...
            <div class="message-thread" id="messageThread">
                <?php foreach ($messages as $message): ?>
                    <div class="message-bubble <?= (int) $message['sender_id'] === (int) $user['id'] ? 'mine' : '' ?>">
                        <span class="message-meta"><?= e($message['fullname']) ?> · <?= e($message['created_at']) ?></span>
                        <div class="<?= $message['body'] === 'Message deleted' ? 'muted deleted-message' : '' ?>"><?= nl2br(e($message['body'])) ?></div>
                ...
                <?php endforeach; ?>
                <?php if (!$messages): ?><p class="muted">Send the first message.</p><?php endif; ?>
            </div>
