...
$items = $isRecipient ? Queue::all((int) $user['id']) : Queue::all(null, $user['role']);

ob_start();
?>
<section class="page-head reveal">
    <div>
        <span class="eyebrow"><?= $isRecipient ? 'Updates' : 'Queue Desk' ?></span>
        <h1><?= $isRecipient ? 'Aid Updates' : 'Coordination Queue' ?></h1>
        <p class="lead">
            <?= $isRecipient
...
<section class="panel reveal">
    <h2><?= $isRecipient ? 'My Updates' : 'Queue Records' ?></h2>
        ...
        <table>
            ...
            <tbody>
                <?php foreach ($items as $item): ?>
                    <tr>
                        <?php if (!$isRecipient): ?><td><?= e($item['fullname']) ?></td><?php endif; ?>
                            ...
                            <?php if (!$isRecipient): ?>
                                <?php if ($item['status'] === 'queued'): ?>
                                    <form method="POST">
                                        ...
                                        <button class="small-button" name="consume">Acknowledge</button>
                                    </form>
                                <?php else: ?>
                                ...
                                <form class="compact-action" method="POST" data-confirm="Delete this queue record?">
                                    ...
                                    <input type="hidden" name="queue_id" value="<?= (int) $item['id'] ?>">
                                    <button class="icon-action danger-icon" type="submit" title="Delete">🗑</button>
                                </form>
                            <?php else: ?>
                ...
                <?php endforeach; ?>
                <?php if (!$items): ?>
                    <tr><td colspan="<?= $isRecipient ? 5 : 6 ?>">No queue entries available.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
